...
const RundownDetailPage = () => {
  ...
  // ===== CONSISTENT PRICE FORMATTING FUNCTIONS (MATCHES YourTripPopup) =====
  
  // Enhanced price parsing function - standardized
  ...
  // Standardized single price formatting - EXACTLY matches YourTripPopup
  ...
  // Standardized price range formatting - EXACTLY matches YourTripPopup
  ...
  const calculateBudgetEstimation = (itinerary) => {
    ...
    // Use consistent price range formatting - UPDATED FROM MANUAL FORMAT
    ...
    if (Array.isArray(itinerary) && itinerary[0]?.destinations) {
      return itinerary.map(day => ({
        ...
        destinations: day.destinations.map(dest => ({
          ...
          // Ensure price is preserved exactly as in YourTripPopup
