...
// // import { Save, Check, User, Login } from 'lucide-react';
...
//   const [popupSize, setPopupSize] = useState("normal"); // 'normal', 'large'
...
//       setIsSaved(false); // Reset saved state when new rundown is generated
//     };
...
//                     destData.category
//                   } - ${destData.name}`,
//                   price: destData.price ? `Rp ${destData.price}` : "Free",
//                   original: destData,
//                 };
//               } else {
//                 return {
...
//     console.log("Opening rundown generator");
...
//   // Handle generating a rundown based on user responses
//   const handleGenerateRundown = (rundown) => {
//     console.log("Generated rundown:", rundown);

//     const validatedRundown = {
//       title: rundown?.title || "Your Trip Rundown",
//       itinerary: Array.isArray(rundown?.itinerary) ? rundown.itinerary : [],
//       budgetEstimation:
//         rundown?.budgetEstimation || calculateBudgetEstimation(),
//       tripDetails: rundown?.tripDetails || { categories: [] },
//       error: rundown?.error || null,
//     };

//     setGeneratedRundown(validatedRundown);
//     setIsSaved(false); // Reset saved state for new rundown

//     // Save rundown to localStorage for persistence
...
//       if (dest.price && dest.price.includes("Rp")) {
//         const priceMatch = dest.price.match(/(\d+[\d.,]*)/g);
//         if (priceMatch && priceMatch.length > 0) {
//           const basePrice =
//             parseInt(priceMatch[0].replace(/[.,]/g, ""), 10) || 0;
...
//     console.log(generatedRundown); //debug
//     console.log(selectedDestinations); //debug
//     if (
...
//     // Create a map of destination names to prices for easy lookup
//     const destinationPriceMap = {};
//     selectedDestinations.forEach(dest => {
//       const name = dest.original?.name || dest.label.split('-').pop().trim();
//       destinationPriceMap[name] = dest.price;
//     });

//     // Enhance itinerary items with prices
//     const enhancedItinerary = rundown.itinerary.map(item => {
//       const activityName = item.activity.split('-').pop().trim();
//       return {
//         ...item,
//         price: destinationPriceMap[activityName] || 'Harga tidak tersedia'
//       };
//     });
...
//               if (destDoc.exists()) {
//                 const destData = destDoc.data();
//                  console.log("Destinasiiii Data: ", destData); // Log data untuk memeriksa harga
//                 return {
//                   id: item.id,
//                   label: `${getCategoryEmoji(destData.category)} ${
//                     destData.category
...
//   // Improved handleGenerateRundown with better price mapping
//   const handleGenerateRundown = (rundown) => {
//     console.log("Generated rundown raw:", rundown);
//     console.log("Selected destinations:", selectedDestinations);

//     // 1. Create a normalized price map
//     const priceMap = {};
//     selectedDestinations.forEach(dest => {
//       // Get the clean destination name (remove emoji and category)
//       const cleanName = dest.label.replace(/^[^\w]*\s*[^-]+\s*-\s*/, '').trim();
//       const originalName = dest.original?.name || cleanName;
      
//       // Use original price if available, otherwise use formatted price
//       const price = dest.original?.price 
//         ? `Rp ${Number(dest.original.price).toLocaleString('id-ID')}`
//         : dest.price || 'Harga tidak tersedia';
      
//       // Add both original name and clean name to the map
//       priceMap[originalName.toLowerCase()] = price;
//       priceMap[cleanName.toLowerCase()] = price;
//     });

//     console.log("Price mapp:", priceMap);

//     // 2. Enhance itinerary with prices
//     const enhancedItinerary = rundown.itinerary.map(item => {
//       // Extract the most likely destination name from activity
//       let activityName = item.activity
//         .replace(/^[^a-zA-Z0-9]*/, '') // Remove leading symbols
//         .split('-')[0]                 // Take first part before hyphen
//         .trim();
      
//       // Try to find matching price (case insensitive)
//       // const matchedPrice = priceMap[activityName.toLowerCase()] || 
//       //                    Object.entries(priceMap).find(([name]) => 
//       //                      item.activity.toLowerCase().includes(name.toLowerCase())
//       //                    )?.[1] || 
//       //                    'Hargaaa tidak tersedia';
//       // const matchedPrice = dest.original?.price; 

//       console.log(`Mapping activity: "${item.activity}" -> "${activityName}" -> ${matchedPrice}`);
